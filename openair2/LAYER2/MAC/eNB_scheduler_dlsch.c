  ...
  for (i = 0; i < sli->n_dl; i++) {
    ...
    if (sli->dl[i].isol_current != sli->dl[i].isol) {
      if (sli->dl[i].isol != 1 && sli->dl[i].isol != 0) {
        ...
        LOG_N(MAC, "[eNB %d][SLICE %d][DL] frame %d subframe %d: slice isolation setting has changed (%x-->%x)\n",
    ...
    if (sli->dl[i].prio_current != sli->dl[i].prio) {
      LOG_N(MAC, "[eNB %d][SLICE %d][DL] frame %d subframe %d: slice priority setting has changed (%d-->%d)\n",
...
#warning RK->CR This old API call has to be revisited for FAPI, or logic must be changed
      ...
      add_ue_spec_dci(&eNB->common_channels[CC_id].DCI_pdu, &(char[]) {0}, rnti, 1, aggregation, 1, format1, 0);
#endif
	...
	if (ta_len + sdu_length_total + header_length_total > 0) {
	    ...
	    UE_list->DLSCH_pdu[CC_id][0][UE_id].payload[0][offset + sdu_length_total + j] = 0;
          ...
          if (opt_enabled == 1) {
            trace_pdu(1, (uint8_t *) UE_list->DLSCH_pdu[CC_id][0][UE_id].payload[0],
                      TBS, module_idP, 3, UE_RNTI(module_idP, UE_id), eNB->frame,
                      eNB->subframe, 0, 0);
            LOG_D(OPT,
	  ...
	  eNB_UE_stats = &UE_list->eNB_UE_stats[CC_id][UE_id];
          ...
          int32_t framex10psubframe = UE_list->UE_template[CC_id][UE_id].pucch_tpc_tx_frame * 10 +
                                      UE_list->UE_template[CC_id][UE_id].pucch_tpc_tx_subframe;
